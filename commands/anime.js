...
exports.run = async (client, message, args, prefix) => { //From here our command starts
    ...
    try {
		...
		if (!body.data.length) return message.reply('Böyle bir anime bulamadım'); //If your search is missing or wrong, it does not support most languages
		...
		const embed = new Discord.MessageEmbed() //Let's see the search results
			...
			.addField('❯ Tür', `${data.showType} - ${data.status}`, true) //To tell you its type, manga, movie or anime; With his statuts if it is continuous or finished
			.addField('❯ Bölümler', data.episodeCount || '???', true) //To tell you how many episodes it has, it may not tell you the number of episodes of most of the episodes if their episodes are not specified in total
			.addField('❯ Başlama Tarihi', data.startDate ? new Date(data.startDate).toDateString() : '???', true) //When did it started
			.addField('❯ Bitiş Tarihi', data.endDate ? new Date(data.endDate).toDateString() : '???', true); //When did it ended
		return message.channel.send(embed);
	} catch (err) {
		return message.reply(`Bazı şeyler yanlış gitti: \`${err.message}\`. sonra tekrar deneyin!`); //Let's check if your search has a bug
	}
};
exports.conf = {
    enabled: true,
