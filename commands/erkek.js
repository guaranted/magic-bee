...
if(!args[0] || !message.mentions.members.first()) return message.channel.send(nn.setTitle('Bir hata oldu!').setColor('RED').setDescription(`Kullanıcı Etiketleyerek dener misin?
**Örnek:**
\`\`\`${client.ayarlar.prefix}erkek @etiket
${client.ayarlar.prefix}erkek 686185592899633200
${client.ayarlar.prefix}erkek 686185592899633200\`\`\``)).then(a => a.delete({timeout: 10000}));
...
if(!member.roles.cache.has(kayıtsızroleID)) return message.channel.send(nn.setColor('RED').setDescription(`${kayıtsız} | Bu Rol kayıt edilecek üyenin üzerinde bulunması gerekiyor.`));
